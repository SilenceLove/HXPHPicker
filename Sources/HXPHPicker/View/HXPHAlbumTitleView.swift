...
class HXPHAlbumTitleView: UIControl {
   
   var config: HXPHAlbumTitleViewConfiguration
   ...
   lazy var arrowView: HXPHAlbumTitleArrowView = {
       let arrowView = HXPHAlbumTitleArrowView.init(frame: CGRect(x: titleLb.frame.maxX + 5, y: 0, width: 20, height: 20), config: self.config)
       
       return arrowView
   }()
   
   init(config: HXPHAlbumTitleViewConfiguration) {
       self.config = config
