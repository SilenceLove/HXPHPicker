...
protocol HXVideoEditorCropViewDelegate: NSObjectProtocol {
...
class HXVideoEditorCropView: UIView {
    
    weak var delegate: HXVideoEditorCropViewDelegate?
    ...
    var avAsset: AVAsset
    var config: HXVideoCroppingConfiguration
    
    var videoFrameCount: Int = 0
    lazy var frameMaskView: HXVideoEditorFrameMaskView = {
        let frameMaskView = HXVideoEditorFrameMaskView.init()
    ...
    lazy var collectionView: UICollectionView = {
        ...
        collectionView.register(HXVideoEditorCropViewCell.self, forCellWithReuseIdentifier: "HXVideoEditorCropViewCellID")
    ...
    init(avAsset: AVAsset, config: HXVideoCroppingConfiguration) {
...
// MARK: function
extension HXVideoEditorCropView {
    ...
    func updateTimeLabels() {
        ...
        startTimeLb.text = HXPHTools.transformVideoDurationToString(duration: TimeInterval(round(startDuration)))
        endTimeLb.text = HXPHTools.transformVideoDurationToString(duration: TimeInterval(round(endDuration)))
        totalTimeLb.text = HXPHTools.transformVideoDurationToString(duration: TimeInterval(round(endDuration - startDuration)))
    }
...
extension HXVideoEditorCropView: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        videoFrameCount
    }
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "HXVideoEditorCropViewCellID", for: indexPath) as! HXVideoEditorCropViewCell
        return cell
    }
    ...
    func setCurrentCell(image: UIImage, index: Int) {
        DispatchQueue.main.async {
            let cell = self.collectionView.cellForItem(at: IndexPath(item: index, section: 0)) as? HXVideoEditorCropViewCell
            cell?.image = image
        }
    }
...
extension HXVideoEditorCropView: HXVideoEditorFrameMaskViewDelegate {
    func frameMaskView(validRectDidChanged frameMaskView: HXVideoEditorFrameMaskView) {
        delegate?.cropView(self, didChangedValidRectAt: getStartTime())
        updateTimeLabels()
    }
    func frameMaskView(validRectEndChanged frameMaskView: HXVideoEditorFrameMaskView) {
